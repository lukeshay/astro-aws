...
import { BucketDeployment, Source, type BucketDeploymentProps } from "aws-cdk-lib/aws-s3-deployment";
...
type AstroAWSS3BucketDeploymentCdkProps = {
	s3BucketDeployment?: Partial<Omit<BucketDeploymentProps, "destinationBucket" | "distribution">>;
};
...
> {
	...
	public constructor(scope: Construct, id: string, props: AstroAWSS3BucketDeploymentProps) {
		super(scope, id, props);

		const { output, distDir, bucket, cdk = {}, distribution } = this.props;

		const source = ["server", "edge"].includes(output) ? resolve(distDir, "client") : resolve(distDir);

		this.#s3BucketDeployment = new BucketDeployment(this, "BucketDeployment", {
			...cdk.s3BucketDeployment,
			destinationBucket: bucket,
			distribution,
			sources: [Source.asset(source), ...(cdk.s3BucketDeployment?.sources ?? [])],
		});
	}
