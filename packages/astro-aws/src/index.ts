...
export const astroAWSFunctions = ({ binaryMediaTypes }: AstroAWSOptions = {}): AstroIntegration => {
		...
		hooks: {
			...
			"astro:config:done": ({ config, setAdapter }) => {
				...
				if (config.output === "static") {
					console.warn(`[astro-aws] \`output: "server"\` is required to use this adapter.`);
					console.warn(`[astro-aws] Otherwise, this adapter is not required to deploy a static site to Netlify.`);
				}
			},
			...
			"astro:build:done": async ({ routes }) => {
				await writeFile(fileURLToPath(getBuildPath(_config.root, "./routes.json")), JSON.stringify(routes, null, 2));

				await bundleEntry(
					fileURLToPath(new URL(_config.build.serverEntry, _config.build.server)),
