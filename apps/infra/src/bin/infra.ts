...
const createStackName = (environment: string, stack: string) => `AstroAWS-${environment}-${stack}`;

Object.entries(ENVIRONMENT_PROPS).forEach(([environment, environmentProps]) => {
	const monitoringStack = new MonitoringStack(app, createStackName(environment, "Monitoring"), environmentProps);
